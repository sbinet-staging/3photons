...
PROGRAM GQ58X3_Example
   ...
   DO i=1,N
     sum = sum + GQ58X3_generate (state)
   END DO

   WRITE(*,1) N
   rsum = sum/4294967296.
   val = GQ58X3_generate (state)/4294967296.
   IF (rsum<0) THEN
     rsum=rsum+1
   ENDIF
   IF (val<0) THEN
...
1  FORMAT(I9," GQ58X3 pseudorandom numbers generated using general instructions of CPU.")
