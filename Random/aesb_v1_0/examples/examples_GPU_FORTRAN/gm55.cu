...
extern "C" __host__ unsigned int gm55_sse_generate_(gm55_sse_state* state){
  unsigned output;
...
extern "C" __device__ __host__ void gm55_get_sse_state_(gm55_state* state,gm55_sse_state* sse_state){
  int i; for(i=0;i<8;i++) {sse_state->xN[i]=state->xN[i]; sse_state->xP[i]=state->xP[i];}
}

extern "C" __device__ __host__ lt gm55_mod_g(lt x){ // returns x (mod g)
  lt F,G; F = (x>>55); G = x-(F<<55)+(2064*F);
  return ((G>=gm55_g) ? (G-gm55_g) : G);
}

extern "C" __device__ __host__ lt gm55_MyMult(lt A,lt B){ // returns AB (mod gm55_g), where it is implied that A,B<gm55_g;
...
extern "C" __device__ __host__ lt gm55_CNext2(lt N,lt P,lt myk,lt myq){   // returns (myk*N-myq*P) (mod gm55_g)
  lt curr1,curr2;
  curr1=gm55_MyMult(myk,N); curr2=gm55_MyMult(myq,P);
  if(curr1>=curr2) return (curr1-curr2); else return (gm55_g+curr1-curr2);
}

extern "C" __device__ __host__ lt gm55_CNext(lt N,lt P){ // returns (256*N-176*P) (mod gm55_g)
  return gm55_mod_g((N<<8)+176*(gm55_g-P));
}

extern "C" __device__ __host__ lt gm55_GetNextN(lt x0,lt x1,unsigned int n){ //returns x_{2^n}
...
extern "C" __device__ __host__ lt gm55_GetNextAny(lt x0,lt x1,lt N64,lt N0){ //N=2^64*N64+N0+1
...
extern "C" __device__ __host__ void gm55_skipahead_(gm55_state* state, lt offset64, lt offset0){ // offset=offset64*2^64+offset0+1
  lt xn,xp,j; 
  for(j=0;j<8;j++){
...
extern "C" __device__ __host__ void gm55_init_(gm55_state* state){
...
extern "C" __device__ __host__ void gm55_init_short_sequence_(gm55_state* state,lt SequenceNumber){ // 0 <= SequenceNumber < 10^18
...
extern "C" __device__ __host__ void gm55_init_long_sequence_(gm55_state* state,lt SequenceNumber){ // 0 <= SequenceNumber < 4*10^9
  gm55_init_(state);                                                      // length of each sequence  <   10^20
  gm55_skipahead_(state,8*SequenceNumber,2699204111*SequenceNumber);
}

extern "C" __device__ __host__ unsigned int gm55_generate_(gm55_state* state){
...
extern "C" __device__ __host__ float gm55_generate_uniform_float_(gm55_state* state){
  unsigned int sum=0; int i; lt temp;
  for(i=0;i<8;i++){ 
    temp=gm55_mod_g(((state->xN[i])<<8)+gm55_q*(gm55_g-state->xP[i]));
    state->xP[i]=state->xN[i]; state->xN[i]=temp;
...
extern "C" __host__ void gm55_print_state_(gm55_state* state){int i;
...
extern "C" __host__ void gm55_print_sse_state_(gm55_sse_state* state){int i;
    printf("Generator State:\nxN={");
    for(i=0;i<8;i++) {printf("%llu",state->xN[i]%gm55_g); printf((i<7)?",":"}\nxP={");}
    for(i=0;i<8;i++) {printf("%llu",state->xP[i]%gm55_g); printf((i<7)?",":"}\n\n");}
}
...
extern "C" __host__ void gm55_generate_gpu_array_(gm55_state* state, unsigned int* dev_out, unsigned int* length){
...
extern "C" __host__ void gm55_generate_gpu_array_float_(gm55_state* state, float* dev_out, unsigned int* length){
...
extern "C" __host__ void gm55_generate_gpu_array_double_(gm55_state* state, double* dev_out, unsigned int* length){
...
extern "C" __host__ void gm55_generate_array_(gm55_state* state, unsigned int* out, unsigned int* length){
