...
extern "C" __host__ unsigned int gq58x4_sse_generate_(gq58x4_sse_state* state){
  unsigned output;
...
extern "C" __device__ __host__ void gq58x4_get_sse_state_(gq58x4_state* state,gq58x4_sse_state* sse_state){
  int i; for(i=0;i<8;i++) {sse_state->xN[i]=state->xN[i]; sse_state->xP[i]=state->xP[i];}
}


extern "C" __device__ __host__ lt gq58x4_mod_g(lt x){ // returns x (mod g)
  lt F,G; F = (x>>58); G = x-(F<<58)+(F<<29)+(F<<30);
  return ((G>=gq58x4_g) ? (G-gq58x4_g) : G);
}

extern "C" __device__ __host__ lt gq58x4_MyMult(lt A,lt B){ // returns AB (mod gq58x4_g), where it is implied that A,B<gq58x4_g;
...
extern "C" __device__ __host__ lt gq58x4_CNext2(lt N,lt P,lt myk,lt myq){   // returns (myk*N-myq*P) (mod gq58x4_g)
  lt curr1,curr2;
  curr1=gq58x4_MyMult(myk,N); curr2=gq58x4_MyMult(myq,P);
  if(curr1>=curr2) return (curr1-curr2); else return (gq58x4_g+curr1-curr2);
}

extern "C" __device__ __host__ lt gq58x4_CNext(lt N,lt P){ // returns (8N-48P) (mod gq58x4_g)
  return gq58x4_mod_g((N+6*(gq58x4_g-P))<<3);
}


extern "C" __device__ __host__ lt gq58x4_GetNextN(lt x0,lt x1,unsigned int n){ //returns x_{2^n}
...
extern "C" __device__ __host__ lt gq58x4_GetNextAny(lt x0,lt x1,lt N64,lt N0){ //N=2^64*N64+N0+1
...
extern "C" __device__ __host__ void gq58x4_skipahead_(gq58x4_state* state, lt offset64, lt offset0){ // offset=offset64*2^64+offset0+1
  lt xn,xp,j; 
  for(j=0;j<8;j++){
...
extern "C" __device__ __host__ void gq58x4_init_(gq58x4_state* state){
...
extern "C" __device__ __host__ void gq58x4_init_short_sequence_(gq58x4_state* state,unsigned SequenceNumber){
...
extern "C" __device__ __host__ void gq58x4_init_medium_sequence_(gq58x4_state* state,unsigned SequenceNumber){
...
extern "C" __device__ __host__ void gq58x4_init_long_sequence_(gq58x4_state* state,unsigned SequenceNumber){
...
extern "C" __device__ __host__ unsigned int gq58x4_generate_(gq58x4_state* state){
...
extern "C" __device__ __host__ float gq58x4_generate_uniform_float_(gq58x4_state* state){
  unsigned int sum=0; int i; lt temp;
  for(i=0;i<8;i++){ 
    temp=gq58x4_mod_g((state->xN[i]+6*(gq58x4_g-state->xP[i]))<<3);
    state->xP[i]=state->xN[i]; state->xN[i]=temp;
...
extern "C" __host__ void gq58x4_print_state_(gq58x4_state* state){int i;
...
extern "C" __host__ void gq58x4_print_sse_state_(gq58x4_sse_state* state){int i;
    printf("Generator State:\nxN={");
    for(i=0;i<8;i++) {printf("%llu",state->xN[i]%gq58x4_g); printf((i<7)?",":"}\nxP={");}
    for(i=0;i<8;i++) {printf("%llu",state->xP[i]%gq58x4_g); printf((i<7)?",":"}\n\n");}
}
...
extern "C" __host__ void gq58x4_generate_gpu_array_(gq58x4_state* state, unsigned int* dev_out, unsigned int* length){
...
extern "C" __host__ void gq58x4_generate_gpu_array_float_(gq58x4_state* state, float* dev_out, unsigned int* length){
...
extern "C" __host__ void gq58x4_generate_gpu_array_double_(gq58x4_state* state, double* dev_out, unsigned int* length){
...
extern "C" __host__ void gq58x4_generate_array_(gq58x4_state* state, unsigned int* out, unsigned int* length){
